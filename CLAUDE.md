# AGENTS.md - AI Agent Development Guide for tap-survicate
...
This tap follows the Singer specification and uses the Meltano Singer SDK to extract data from Survicate.

### Key Components

1. **Tap Class** (`tap_survicate/tap.py`): Main entry point, defines streams and configuration
1. **Client** (`tap_survicate/client.py`): Handles API communication and authentication
1. **Streams** (`tap_survicate/streams.py`): Define data streams and their schemas
...
1. Define stream class in `tap_survicate/streams.py`
...
```python
class MyNewStream(SurvicateStream):
    name = "my_new_stream"
...
1. **SimpleHeaderPaginator**: For APIs using Link headers (RFC 5988)
   ...
   class MyStream(SurvicateStream):
...
```bash
tap-survicate --config config.json --discover
tap-survicate --config config.json --catalog catalog.json
```
...
1. Update `config_jsonschema` in `tap_survicate/tap.py`
...
```bash
# .env.example
TAP_SURVICATE_API_URL=https://api.example.com
TAP_SURVICATE_API_KEY=your_api_key_here
TAP_SURVICATE_BATCH_SIZE=100  # New setting
```
...
Before filing, search existing issues to avoid duplicates. Include the SDK version (`uv run tap-survicate --version`), Python version, and a minimal reproduction case when reporting bugs.
